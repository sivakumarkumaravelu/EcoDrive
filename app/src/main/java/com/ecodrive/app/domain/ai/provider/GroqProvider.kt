...
class GroqProvider @Inject constructor() : AiProvider {
    ...
    override val name: String = "GROQ" // Keep name as GROQ for backward compatibility with settings
    override val defaultModel: String = "grok-2-latest"
    ...
    companion object {
        ...
        private const val MODELS_URL = "https://api.x.ai/v1/models"
    }
    ...
    override suspend fun getAvailableModels(): List<String>? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.w(TAG, "Grok models fetch failed: ${e.message}")
            null
        }
    }
