...
@Singleton
class MistralProvider @Inject constructor() : AiProvider {
    ...
    companion object {
        ...
        private const val MODELS_URL = "https://api.mistral.ai/v1/models"
    }

    override suspend fun getAvailableModels(): List<String>? = withContext(Dispatchers.IO) {
        val apiKey = AiConfig.MISTRAL_API_KEY
        ...
        try {
            ...
            Log.w(TAG, "Mistral models fetch failed: ${e.message}")
            null
        }
    }
    ...
    private suspend fun generate(prompt: String, temp: Float, maxTokens: Int, model: String?): String? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.w(TAG, "Mistral failed: ${e.message}")
            null
        }
    }
}
